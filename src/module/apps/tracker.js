export class DuneTracker extends Application {
  ...
  activateListeners(html) {
    let threat = game.settings.get('dune', 'threat');
    let momentum = game.settings.get('dune', 'momentum');
    renderTracker();
    this.checkUpdates();

    if (!game.user.hasRole(game.settings.get('dune', 'threatPermissionLevel'))) {
    ...
    if (!game.user.hasRole(game.settings.get('dune', 'momentumPermissionLevel'))) {
    ...
    html.find('#sta-momentum-track-decrease > .fas').click((ev) => {
      threat = game.settings.get('dune', 'threat');
      ...
      momentum = momentum - 1;
      game.settings.set('dune', 'momentum', momentum);
      renderTracker();
    });

    html.find('#sta-threat-track-decrease > .fas').click((ev) => {
      momentum = game.settings.get('dune', 'momentum');
      ...
      threat = threat - 1;
      game.settings.set('dune', 'threat', threat);
      renderTracker();
    });
  ...
  async checkUpdates() {
    const refreshRate = game.settings.get('dune', 'trackerRefreshRate') * 1000;
    function check() {
      ...
      const storedThreat = game.settings.get('dune', 'threat');
      const storedMomentum = game.settings.get('dune', 'momentum');
